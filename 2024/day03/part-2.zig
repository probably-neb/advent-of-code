...
pub fn main() !void {
    ...
    while (char_index < (input.len - "mul(x,x)".len)) : (char_index += 1) {
        if (std.mem.eql(u8, "do()", input[char_index..][0.."do()".len])) {
            enabled = true;
            continue;
        }
        if (std.mem.eql(u8, "don't()", input[char_index..][0.."don't()".len])) {
            enabled = false;
            continue;
        }
        if (!enabled) {
        ...
        const a = try std.fmt.parseInt(u32, dbg("a: {s}\n", mul["mul(".len..comma_index]), 10);
        const b = try std.fmt.parseInt(u32, dbg("b: {s}\n", mul[comma_index + 1 .. mul.len - 1]), 10);
