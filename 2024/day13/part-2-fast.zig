...
const input = @embedFile("./input-smol-AABA.txt");
...
pub fn main() !void {
    ...
    for (0..C.h) |_row| {
        ...
        for (0..C.w) |_col| {
            ...
            const corner_board = try alloc.alloc(u8, input.len);
            @memset(corner_board, '0');

            for (0..C.h) |_r| {
                for (0..C.w) |_c| {
                    const q_row: u32 = @intCast(_r);
                    const q_col: u32 = @intCast(_c);
                    const q_loc = C.at(q_row, q_col);
                    const e = edges[q_loc];
                    const i_c = corners;
                    if (e.u and e.l) {
                    ...
                    if (q_col < C.w - 1 and edges[C.right(q_row, q_col)].u and q_row > 0 and edges[C.up(q_row, q_col)].r) {
                        corners += 1;
                    }
                    if (q_col < C.w - 1 and edges[C.right(q_row, q_col)].d and q_row < C.h - 1 and edges[C.down(q_row, q_col)].r) {
                        corners += 1;
                    }
                    if (q_col > 0 and edges[C.left(q_row, q_col)].d and q_row > 0 and edges[C.up(q_row, q_col)].l) {
                        corners += 1;
                    }
                    if (q_col > 0 and edges[C.left(q_row, q_col)].u and q_row < C.h - 1 and edges[C.down(q_row, q_col)].l) {
                        corners += 1;
                    }
                    if (q_col > 0 and edges[C.left(q_row, q_col)].d and q_row < C.h - 1 and edges[C.down(q_row, q_col)].l) {
                        corners += 1;
                    }

                    corner_board[q_loc] = @as(u8, @intCast(corners - i_c)) + '0';
                }
            }

            if (plant == 'C') {
                std.debug.print("corners: {d}\n", .{corners});
                var vis = try alloc.alloc(bool, visited.len);
                for (0..edges.len) |i| {
                    vis[i] = edges[i].any();
                }
                print_edges(stdout, C, edges) catch unreachable;
                print_board(stdout, C, corner_board, vis) catch unreachable;
                print_board(stdout, C, input, vis) catch unreachable;
                bw.flush() catch unreachable;
            }
            ...
            std.debug.print("edges = {} corners = {} plant = {c}\n", .{ info_ptr.edges, corners, plant });
            std.debug.assert(info_ptr.edges == corners);
        }
    }
